...
startServerBtn.addEventListener("click", async () => {
  ...
  appendLog({
    type: "server-log",
    message: `Attempting to start server on port ${port}...`,
    level: "info",
  });
...
browseDbPathBtn.addEventListener("click", async () => {
  ...
  if (filePath) {
    ...
    appendLog({
      type: "server-log",
      message: `Selected database path: ${filePath}`,
      level: "info",
    });
  }
});
...
saveDbPathBtn.addEventListener("click", async () => {
  ...
  if (newPath) {
    appendLog({
      type: "server-log",
      message: `Saving new database path: ${newPath}...`,
      level: "info",
    });
    ...
    if (result.success) {
      ...
      appendLog({
        type: "server-log",
        message: `Database path successfully saved: ${result.dbPath}`,
        level: "info",
      });
    } else {
      appendLog({
        type: "server-log",
        message: `Failed to save database path.`,
        level: "error",
      });
    }
  } else {
...
importUsersBtn.addEventListener("click", async () => {
  ...
  if (file.type !== "text/csv") {
    appendLog({
      type: "server-log",
      message: "Selected file is not a CSV. Please select a .csv file.",
      level: "warn",
    });
    return;
  }
  ...
  if (!status.isRunning) {
    appendLog({
      type: "server-log",
      message:
        "Server is not running. Please start the server before importing users.",
      level: "error",
    });
    return;
  }
  ...
  appendLog({
    type: "server-log",
    message: `Importing users from ${file.name}...`,
    level: "info",
  });
  ...
  try {
    const response = await fetch(
      `http://localhost:${status.port}/import-users-csv`,
      {
        method: "POST",
        body: formData,
      }
    ...
    if (response.ok) {
      appendLog({
        type: "server-log",
        message: `User import successful: ${data.message}`,
        level: "info",
      });
      if (data.errors && data.errors.length > 0) {
        data.errors.forEach((err) =>
          appendLog({
            type: "server-log",
            message: `Import Warning/Error: ${err}`,
            level: "warn",
          })
        );
      }
      ...
      appendLog({
        type: "server-log",
        message: `User import failed: ${data.error || response.statusText}`,
        level: "error",
      });
      if (data.errors && data.errors.length > 0) {
        data.errors.forEach((err) =>
    ...
    appendLog({
      type: "server-log",
      message: `Network or server error during import: ${error.message}`,
      level: "error",
    });
  } finally {
...
window.electron.onServerStatusUpdate((status) => {
  ...
  appendLog({
    type: "server-log",
    message: `Server status updated to: ${status.status} ${
