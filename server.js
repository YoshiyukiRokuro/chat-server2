...
function initializeDatabase(dbPath, callback) {
  db = new sqlite3.Database(dbPath, (err) => {
    ...
    db.serialize(() => {
      ...
      db.run(
        `CREATE TABLE IF NOT EXISTS channels (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, is_deletable BOOLEAN DEFAULT 1, is_group BOOLEAN DEFAULT 0, creator_id INTEGER, FOREIGN KEY(creator_id) REFERENCES users(id) ON DELETE SET NULL)`
      );
      db.run(
...
function setupApiEndpoints(app) {
  ...
  app.delete("/messages/:id", authenticateToken, (req, res) => {
    db.get(
      ...
      [req.params.id],
      (err, row) => {
        if (err || !row || row.user !== req.user.username)
