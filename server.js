...
const SECRET_KEY =
  process.env.SECRET_KEY || "your-super-secret-key-for-qoler-chat";
...
function initializeDatabase(dbPath, callback) {
  db = new sqlite3.Database(dbPath, (err) => {
    ...
    db.serialize(() => {
      db.run(
        `CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT NOT NULL, password TEXT NOT NULL, UNIQUE(id))`
      );
      db.run(
        `CREATE TABLE IF NOT EXISTS channels (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, is_deletable BOOLEAN DEFAULT 1, is_group BOOLEAN DEFAULT 0, creator_id INTEGER, FOREIGN KEY(creator_id) REFERENCES users(id) ON DELETE SET NULL)`
      );
      db.run(
        `CREATE TABLE IF NOT EXISTS channel_members (channel_id INTEGER NOT NULL, user_id INTEGER NOT NULL, PRIMARY KEY (channel_id, user_id), FOREIGN KEY(channel_id) REFERENCES channels(id) ON DELETE CASCADE, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`
      );
      db.run(
      ...
      ["連絡", "雑談"].forEach((name) =>
        db.run(
          "INSERT OR IGNORE INTO channels (name, is_deletable, is_group) VALUES (?, 0, 0)",
          [name]
        )
      );
      callback(null);
...
function setupApiEndpoints(app) {
  ...
  app.post("/channels", authenticateToken, (req, res) => {
    db.run(
      "INSERT INTO channels (name, is_group) VALUES (?, 0)",
      [req.body.name],
      function (err) {
        if (err)
          return res
            .status(500)
            .json({ error: "チャンネル作成に失敗しました" });
        const newChannel = {
          id: this.lastID,
          name: req.body.name,
          is_group: 0,
  ...
  app.post("/channels/group", authenticateToken, (req, res) => {
    ...
    db.run(
      "INSERT INTO channels (name, is_group, creator_id) VALUES (?, 1, ?)",
      [name, req.user.id],
      function (err) {
        if (err)
  ...
  app.put("/channels/:id/name", authenticateToken, (req, res) => {
    db.run(
      ...
      function (err) {
        if (err)
          return res.status(500).json({ error: "名前の変更に失敗しました" });
  ...
  app.delete("/channels/:id", authenticateToken, (req, res) => {
    db.run(
      "DELETE FROM channels WHERE id = ? AND is_deletable = 1",
      [req.params.id],
      function (err) {
  ...
  app.post("/channels/:id/members", authenticateToken, (req, res) => {
    ...
    req.body.userIds.forEach((userId) => stmt.run(req.params.id, userId));
    stmt.finalize((err) => {
      if (err)
        return res.status(500).json({ error: "メンバー追加に失敗しました" });
      broadcast({
        type: "members_updated",
        data: { channelId: Number(req.params.id) },
      });
  ...
  app.delete("/channels/:id/members", authenticateToken, (req, res) => {
    const placeholders = req.body.userIds.map(() => "?").join(",");
    const sql = `DELETE FROM channel_members WHERE channel_id = ? AND user_id IN (${placeholders})`;
    db.run(sql, [req.params.id, ...req.body.userIds], function (err) {
      if (err)
        return res.status(500).json({ error: "メンバー削除に失敗しました" });
      broadcast({
        type: "members_updated",
        data: { channelId: Number(req.params.id) },
      });
  ...
  app.get("/messages/unread-counts", authenticateToken, (req, res) => {
    ...
    db.all(sql, [req.user.id], (err, rows) => {
      ...
      const counts = rows.reduce(
        (acc, row) => ({ ...acc, [row.channelId]: row.count }),
        {}
      );
      res.json(counts);
    });
    ...
    isChannelMember(channelId, req.user.id, (err, isMember) => {
      ...
      stmt.run(
        ...
        function (err) {
          if (err) return res.status(500).json({ error: err.message });
          db.get(
            `SELECT m1.*, m2.user as repliedToUser, m2.text as repliedToText FROM messages AS m1 LEFT JOIN messages AS m2 ON m1.replyToId = m2.id WHERE m1.id = ?`,
            [this.lastID],
            (err, newMessage) => {
              if (newMessage)
  ...
  app.post("/messages/:channelId/read", authenticateToken, (req, res) => {
    ...
    const stmt = db.prepare(
      `INSERT INTO read_receipts (user_id, channel_id, last_read_message_id) VALUES (?, ?, ?) ON CONFLICT(user_id, channel_id) DO UPDATE SET last_read_message_id = excluded.last_read_message_id`
    );
